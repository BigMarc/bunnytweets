"""High-level posting logic for Threads.net accounts.

Mirrors TwitterPoster — pulls media from Google Drive, rotates content
via least-used-file, picks random titles, and schedules CTA self-comments.
"""
...
class ThreadsPoster:
    """Orchestrates downloading from Drive and posting to Threads."""
    ...
    def run_posting_cycle(self) -> bool:
        ...
        logger.info(f"[{self.account_name}] Starting Threads posting cycle")
        ...
        logger.info(
            f"[{self.account_name}] Posting to Threads with '{chosen_meta['name']}'"
        ...
        self.db.increment_file_use(
            self.account_name, chosen_id, chosen_meta["name"], status=status,
        )
        ...
        if success:
            ...
            if self.notifier:
                self.notifier.alert_post_failed(
                    self.account_name,
                    f"compose_tweet returned False for '{chosen_meta['name']}' on Threads",
                )
    ...
    def run_cta_comment(self) -> bool:
        """Find the account's latest post and comment a CTA text under it."""
        cta_text = self.db.get_random_cta(self.account_name)
        if not cta_text:
            logger.debug(f"[{self.account_name}] No CTA texts configured, skipping")
            return False

        username = self.config.get("threads", {}).get("username", "")
        if not username:
            return False

        post_urls = self.auto.get_latest_tweet_urls(username, limit=1)
        if not post_urls:
            logger.warning(
                f"[{self.account_name}] No Threads posts found for CTA comment"
            )
            return False

        post_url = post_urls[0]
        logger.info(
            f"[{self.account_name}] Commenting CTA on own Threads post: {post_url}"
        )
        success = self.auto.reply_to_tweet(post_url, cta_text)
        if success:
            self.db.update_account_status(
    ...
    def _maybe_schedule_cta(self) -> None:
        ...
        if ctas:
            self.db.update_account_status(self.account_name, cta_pending=1)
